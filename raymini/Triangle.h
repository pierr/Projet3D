...
public:
    ...
    inline Triangle (unsigned int v0, unsigned int v1, unsigned int v2, unsigned int nObj) { init (v0, v1, v2, nObj); }
    inline Triangle (const unsigned int * vp) { init (vp[0], vp[1], vp[2]); }
    inline Triangle (const Triangle & it) { init (it.v[0], it.v[1], it.v[2], it.objectId[0]); }
    ...
    inline unsigned int getObjectId() { return objectId[0];}
protected:
    inline void init (unsigned int v0, unsigned int v1, unsigned int v2) {
        v[0] = v0; v[1] = v1; v[2] = v2;  
    }

    inline void init (unsigned int v0, unsigned int v1, unsigned int v2, unsigned int nObjet) {
        v[0] = v0; v[1] = v1; v[2] = v2;
        objectId[0] = nObjet;
        //std::cout << "nObjet " << nObjet <<  "objId " << getObjectId() << std::endl;
    }
private:
    unsigned int v[3];
